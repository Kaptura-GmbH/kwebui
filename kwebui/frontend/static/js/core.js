...
// `terminated` is set once, permanently, when the server deliberately
// closes this tab's connection for good -- a newer tab replaced it
// (KApp(single_session=True), the default) or the app itself stopped
// (KApp.exit()/Ctrl+C/SIGTERM) -- see websocket.js's markTerminal().
// Part of the reactive state rather than a plain flag so widgets holding
// a live resource can react to it: an imagestream's <img> keeps its own
// open HTTP connection to /stream/, entirely independent of the
// WebSocket, so closing the socket does not stop it pulling frames --
// only the widget dropping that <img> does.
const state = Vue.reactive({ widgets: [], terminated: false });
